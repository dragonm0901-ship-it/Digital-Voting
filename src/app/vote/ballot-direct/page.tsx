...
export default function BallotPage() {
  ...
  const {
    language, selectedPartyDirect, setSelectedPartyDirect, setStep, voterId,
  } = useVotingStore();
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-text-primary mb-2 uppercase tracking-wide">{tr('ballotTitle')}</h2>
      ...
      >
        {PARTIES.map((party) => {
          const isSelected = selectedPartyDirect?.id === party.id;
          return (
            <motion.div
              key={party.id}
              role="radio"
              ...
              tabIndex={0}
              onKeyDown={(e) =>
                (e.key === 'Enter' || e.key === ' ') && setSelectedPartyDirect(party)
              }
              onClick={() => setSelectedPartyDirect(party)}
      ...
      {selectedPartyDirect && (
        ...
        >
          ...
          >
            {language === 'ne' ? 'समानुपातिक मतदानमा जानुहोस्' : 'Proceed to Proportional'}
